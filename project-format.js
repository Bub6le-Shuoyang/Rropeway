...
function normalizeBlock(block) {
  ...
  return { type: 'dialogue', character: String(value.character || '未命名角色'), characterKey: value.characterKey === 'yan' ? 'yan' : 'mei', characterColor: String(value.characterColor || '#f2674f'), portraitPreset: value.portraitPreset ? String(value.portraitPreset) : null, emotion: String(value.emotion || ''), voice: String(value.voice || ''), text: String(value.text || ''), note: String(value.note || ''), portrait: value.portrait ? String(value.portrait) : undefined };
}
